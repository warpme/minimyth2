...
def fix_empty_methods(source):
    ...
    a docstring before we removed docstrings =).
    ...
    for line in source.split('\n'):
        if len(line.strip()) > 0: # Don't look at blank lines
            if just_matched == True:
                ...
                def_indentation_level = len(line) - len(line.strip()) # Just a commment
                just_matched = True
                previous_line = line
            else:
                output += "%s\n" % line # Another one for testing
        else:
            output += "\n"
    return output
...
# Here's an example comment/test case that I found where pyminifier used break
# It broke on commented out code, not actual code which is interesting =)
#if spaceleft < localspacelimit0:
#    tolog("!!FAILED!!1999!! Too little space left...: %d (need > %d kB)" %\
#          (spaceleft, localspacelimit0))
# Taken from:
#   http://svn.usatlas.bnl.gov/viewvc/panda/common-utils/current/commonutils/NOTES_ON_PYMINIFIER?view=markup&pathrev=4076
# Note: Obviously not broken anymore!
...
def main():
    if len(sys.argv) > 1:
        ...
        print "Example:"
        print "  python pyminifier.py pyminifier.py > minified_pyminifier.py"
