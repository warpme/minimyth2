...
void TVRec::HandleStateChange(void)
{
    ...
    {
        int start_delay = eit_start_rand(cardid, eitTransportTimeout);
...
void TVRec::run(void)
{
        ...
        (dvbOpt.dvb_eitscan || get_use_eit(cardid)))
    {
        ...
        else
        {
            ...
            eitScanStartTime = eitScanStartTime.addSecs(eitCrawlIdleStart);
            eitScanEndAT = eitScanStartTime.addSecs(eitCrawlDuration*60);
            eitScanStartAT = eitScanStartTime.addSecs(start_delay);

            // If we are in scan window then delay and randomize start
            if (QDateTime::currentDateTime() > eitScanStartAT && QDateTime::currentDateTime() < eitScanEndAT)
            {
                eitScanStartTime = QDateTime::currentDateTime();
                eitScanStartTime = eitScanStartTime.addSecs(60 + start_delay);
                LOG(VB_GENERAL, LOG_INFO, LOC +
